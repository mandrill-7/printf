...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/09 16:36:29 by fchanal           #+#    #+#             */
/*   Updated: 2017/06/13 11:18:11 by fchanal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

#define PF_0X_VALUE		0x5830

static char		*pf_sharp_conv(char conv[4], unsigned char c)
{
	*(uint16_t *)conv = PF_0X_VALUE;
	conv[2] = FT_HEXU_DIGIT[c / 16];
	conv[3] = FT_HEXU_DIGIT[c % 16];
	return (conv);
}

static char		*pf_classic_conv(char conv[4], unsigned char c)
{
	conv[0] = FT_HEXU_DIGIT[c / 16];
	conv[1] = FT_HEXU_DIGIT[c % 16];
	return (conv);
}

bool			pf_convert_big_w(t_env *env, t_attr *attr)
{
	unsigned char	c;
	char			conv[4];
