...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/13 11:14:52 by fchanal           #+#    #+#             */
/*   Updated: 2017/06/13 11:22:22 by fchanal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		pf_insert_hex(t_env *env, const unsigned char *m, size_t size)
{
	...
	while (size)
	{
		pf_set_char(env, FT_HEXU_DIGIT[*m / 16]);
		pf_set_char(env, FT_HEXU_DIGIT[*m % 16]);
		if (!(++count % 2))
		{
			--space;
...
bool			pf_convert_big_m(t_env *env, t_attr *attr)
{
	void	*m;
	int		count;
